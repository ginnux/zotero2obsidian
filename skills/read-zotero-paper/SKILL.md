...
- 环境变量 `OBSIDIAN_VAULT` 指向 Obsidian vault 路径

可选环境变量：

- `BBT_JSON_RPC_URL`：如果 Better BibTeX JSON-RPC 地址不是默认值，使用该变量或脚本参数覆盖
...
对每个 citekey，先在 `$OBSIDIAN_VAULT/papers/notes/{citekey}.md` 查重。若笔记已存在，告知用户并跳过该文献。
...
```text
$OBSIDIAN_VAULT/assets/pdfs/{citekey}.pdf
```

同时把 Zotero 附件信息缓存到：

```text
$OBSIDIAN_VAULT/.paper-cache/{citekey}_zotero.json
```
...
- 全文：`$OBSIDIAN_VAULT/.paper-cache/{citekey}_text.md`
- 图片和页面渲染：`$OBSIDIAN_VAULT/assets/png/{citekey}/`
...
```text
$OBSIDIAN_VAULT/papers/notes/{citekey}.md
```
...
笔记位于 `papers/notes/`，引用本地图片时使用相对路径：

```markdown
![Figure 1: 方法概览|600](../../assets/png/{citekey}/page_3.png)
```
...
所有新笔记写完后执行 `paper-index` skill 更新 `$OBSIDIAN_VAULT/papers/index/` 下的 `.base` 文件。如果一次读多篇论文，等全部笔记完成后统一更新一次。
...
source: "zotero"
pdf: "[[assets/pdfs/ouyang2026reasoningbank.pdf]]"
tags: [tag1, tag2, tag3]
status: unread
rating:
tldr: "一句话概括核心贡献"
...
- **PDF：** [本地 PDF](../../assets/pdfs/ouyang2026reasoningbank.pdf)
...
![Figure X: 说明|600](../../assets/png/ouyang2026reasoningbank/page_1.png)
*Figure X: 中文说明*

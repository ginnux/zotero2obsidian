...
- MinerU API token（推荐）：`MINERU_API_TOKEN` 或 `MINERU_TOKEN`
- Python 3 + PyMuPDF（MinerU 不可用时的本地回退）：`pip install pymupdf`
...
- `MINERU_API_TOKEN`：配置后 `Z2O_EXTRACTOR=auto` 或 `Z2O_EXTRACTOR=mineru` 会优先使用 MinerU 精准识别
...
- 默认尊重 `.env` 中的 `Z2O_EXTRACTOR`，不要为了规避网络权限手动覆盖成 `pymupdf`。
- 推荐提取策略是先用 MinerU 精准解析；MinerU 未配置、不可用或调用失败时，再回退到 PyMuPDF 本地提取。
- 在 Codex/受限沙箱中，Better BibTeX JSON-RPC 的 `localhost:23119` 访问和 MinerU API 都可能需要网络权限。若 `prepare.sh` / `download.sh` / `extract.sh` 报 `Operation not permitted`、DNS/连接失败、无法访问 Better BibTeX 或无法访问 MinerU API，应立即用同一个命令请求提升权限重跑，并在 justification 中说明需要访问本机 Zotero/Better BibTeX 和 MinerU API。
- 提权重跑时不要改变提取器设置；只有 MinerU 未配置、不可用，或脚本已经自动回退时，才接受本地 PyMuPDF 结果。
...
vault/
├── assets/
│   └── ouyang2026reasoningbank/ # 该 citekey 的 PDF、图片和页面渲染
...
```text
{Z2O_ASSETS_DIR}/{citekey}/{citekey}.pdf
```
...
- 若设置 `Z2O_EXTRACTOR=mineru`，同样优先调用 MinerU，并在 MinerU 不可用时回退到 PyMuPDF
- 若设置 `Z2O_EXTRACTOR=mineru-strict`，MinerU 失败时直接报错，不回退
- 若设置 `Z2O_EXTRACTOR=pymupdf`，只使用本地 PyMuPDF

输出：

- 全文：`{Z2O_TEMP_DIR}/{citekey}_text.md`
- 图片、页面渲染和解析元素：`{Z2O_ASSETS_DIR}/{citekey}/`
...
- 当前 Agent 必须读取 `{Z2O_TEMP_DIR}/{citekey}_text.md`、`{Z2O_TEMP_DIR}/{citekey}_zotero.json` 和 `{Z2O_ASSETS_DIR}/{citekey}/` 后自行写笔记
...
笔记位于 `Z2O_NOTES_DIR`，引用本地图片时使用从笔记目录到 `Z2O_ASSETS_DIR/{citekey}` 的相对路径：

```markdown
![Figure 1: 方法概览|600]({relative_path_to_assets_citekey}/page_3.png)
```
...
- **PDF：** [本地 PDF]({relative_path_to_assets_citekey}/ouyang2026reasoningbank.pdf)
...
![Figure X: 说明|600]({relative_path_to_assets_citekey}/page_1.png)
*Figure X: 中文说明*
